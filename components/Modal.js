...
const Modal = forwardRef(({ title, children, ...rest }, ref) => {
  ...
  const leftLine = <View style={[StyleSheet.absoluteFill, { height: 30, top: 10, width: 4, backgroundColor: theme.colors.primary, left: -15 }]}></View>
      ...
      <View style={[theme.modal]}>
        <View style={[theme.modalContainer]}>
          <View style={[theme.modalTitle]}>
            {leftLine}
            <Typography style={[theme.subtitle, { color: theme.colors.primary }]}>{title}</Typography>
            <TouchableOpacity activeOpacity={0.5} style={{ padding: 0, color: theme.colors.primary }} onPress={onClose}>
